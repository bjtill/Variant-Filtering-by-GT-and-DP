...
// Reads one full line of arbitrary length from a gzFile.
// gzgets() only fills a fixed-size buffer and stops at the buffer limit
// even if no newline was found yet, silently truncating long lines.
// GATK multi-sample VCFs routinely produce lines far longer than a
// modest fixed buffer (PL fields grow combinatorially with allele
// count), so we loop, growing the accumulated string, until we see a
// real newline or hit end-of-file.
// Reads one full line of arbitrary length from a gzFile.
// Returns false only on genuine end-of-file (no more data at all).
// Returns true for every real line read, including legitimately
// empty lines, so callers can distinguish "EOF" from "blank line"
...
bool readLine(gzFile file, std::string& line) {
    ...
    while (gzgets(file, buffer, CHUNK_SIZE) != nullptr) {
        got_any_data = true;
        line += buffer;
        // If we captured a trailing newline, this line is complete.
        if (!line.empty() && line.back() == '\n') {
            line.pop_back();
            if (!line.empty() && line.back() == '\r') {
                line.pop_back(); // tolerate CRLF-terminated input
            }
            return true;
        }
        // Otherwise the buffer was exactly filled without hitting a
        // newline yet, meaning the line continues -- loop and append
        // the next chunk instead of returning a truncated fragment.
    }

    if (got_any_data) {
        // Last line in the file had no trailing newline.
        if (!line.empty() && line.back() == '\r') {
            line.pop_back();
        }
        return true;
    }
    return false; // genuine EOF, nothing read at all
}
...
int main(int argc, char* argv[]) {
    ...
    // Process VCF file
    while (readLine(vcf, line)) {
        if (line.empty()) continue; // tolerate genuinely blank lines
